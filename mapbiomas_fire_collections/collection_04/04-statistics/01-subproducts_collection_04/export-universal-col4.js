...
var RECORTES_ATIVOS = [
  'bioma_estado',
  // 'bioma_municipio',
  // 'bioma_tis',         // Terras Indígenas
  // 'bioma_ucs_federal', // Unidades de Conservação Federais
  // 'bioma_ucs_estadual' // Unidades de Conservação Estaduais
];
...
lulc = lulc.addBands(lulc.slice(-1));
...
var LEGENDS = {
  ...
  ucs_estadual_pi: ee.Dictionary(legends_terr.get('estadual_ucs_pi_name')),
  ucs_estadual_us: ee.Dictionary(legends_terr.get('estadual_ucs_us_name')),
...
var DATASETS = {
  1: {
    ...
    decode: function (val) {
      ...
      return ee.Dictionary({ 'Mês': m, 'Meses': LEGENDS.months.get(m), 'Situação': LEGENDS.situation.get(m.gte(1)), 'Classe': l, 'Nível 1': LEGENDS.nivel_1.get(l) });
    },
    columns: ['Mês', 'Meses', 'Situação', 'Classe', 'Nível 1']
  },
  2: {
    ...
    decode: function (val) {
      ...
      return ee.Dictionary({ 'Situação': LEGENDS.situation.get(f.gte(1)), 'Classe': l, 'Nível 1': LEGENDS.nivel_1.get(l) });
    },
    columns: ['Situação', 'Classe', 'Nível 1']
  },
  3: {
    ...
    decode: function (val) {
      ...
      return ee.Dictionary({ 'Acumulado': a, 'Classe': l, 'Nível 1': LEGENDS.nivel_1.get(l) });
    },
    columns: ['Acumulado', 'Classe', 'Nível 1']
  },
  4: {
    ...
    decode: function (val) {
      val = ee.Number(val); var f = val.divide(100).int(); var l = val.mod(100).int();
      return ee.Dictionary({ 'Frequência': f, 'Classe': l, 'Nível 1': LEGENDS.nivel_1.get(l) });
    },
    columns: ['Frequência', 'Classe', 'Nível 1']
  },
  5: {
    ...
    decode: function (val) {
      ...
      return ee.Dictionary({ 'Tamanho N1': LEGENDS.scar_n1.get(s), 'Tamanho N2': LEGENDS.scar_n2.get(s), 'scar_range_id': s, 'Classe': l, 'Nível 1': LEGENDS.nivel_1.get(l) });
    },
    columns: ['Tamanho N1', 'Tamanho N2', 'scar_range_id', 'Classe', 'Nível 1']
  },
  6: {
...
var SPATIAL_UNITS = {
  'bioma_estado': {
    name: 'Bioma_Estado',
    image: (function () {
      var rFC = ee.FeatureCollection('users/geomapeamentoipam/AUXILIAR/regioes_biomas_col2');
      ...
      return ee.Image().paint(rFC, 'region').multiply(100).add(ee.Image().paint(sFC, 'id'));
    })(),
    decode: function (val) {
      val = ee.Number(val); var ri = val.divide(100).int(); var si = val.mod(100).int();
      return ee.Dictionary({ 'Bioma': LEGENDS.biomes.get(ri.divide(10).int()), 'Região': LEGENDS.regions.get(ri), 'Estado': LEGENDS.estados.get(si), 'UF': LEGENDS.ufs.get(si) });
    },
    columns: ['Bioma', 'Região', 'Estado', 'UF']
  },
  'bioma_municipio': {
    ...
    image: (function () {
      ...
      return ee.Image().paint(rFC, 'region').multiply(10000000).add(ee.Image().paint(mFC, 'id'));
    })(),
    decode: function (val) {
      val = ee.Number(val); var ri = val.divide(10000000).int(); var mi = val.mod(10000000).int();
      return ee.Dictionary({ 'Bioma': LEGENDS.biomes.get(ri.divide(10).int()), 'Região': LEGENDS.regions.get(ri), 'Município': LEGENDS.mun_names.get(mi), 'Cod Município': mi });
    },
    columns: ['Bioma', 'Região', 'Município', 'Cod Município']
  },
  'bioma_tis': {
    name: 'Bioma_TI',
    image: (function () {
      ...
      return ee.Image().paint(rFC, 'region').multiply(10000).add(ee.Image().paint(tFC, 'id'));
    })(),
    decode: function (val) {
      val = ee.Number(val); var ri = val.divide(10000).int(); var ti = val.mod(10000).int();
      return ee.Dictionary({ 'Bioma': LEGENDS.biomes.get(ri.divide(10).int()), 'Região': LEGENDS.regions.get(ri), 'Território': 'TI', 'Nome': LEGENDS.tis_names.get(ti), 'Código': ti });
    },
    columns: ['Bioma', 'Região', 'Território', 'Nome', 'Código']
  },
  'bioma_ucs_federal': {
    name: 'Bioma_UC_Federal',
    image: (function () {
      var rFC = ee.FeatureCollection('users/geomapeamentoipam/AUXILIAR/regioes_biomas_col2');
      ...
      var ucs_img = ee.Image().paint(piFC, 'id').unmask(ee.Image().paint(usFC, 'id'));
      return ee.Image().paint(rFC, 'region').multiply(10000).add(ucs_img);
    })(),
    decode: function (val) {
      val = ee.Number(val); var ri = val.divide(10000).int(); var ui = val.mod(10000).int();
      var name = LEGENDS.ucs_federal_pi.get(ui, LEGENDS.ucs_federal_us.get(ui, 'Não identificado'));
      return ee.Dictionary({ 'Bioma': LEGENDS.biomes.get(ri.divide(10).int()), 'Região': LEGENDS.regions.get(ri), 'Território': 'UC Federal', 'Nome': name, 'Código': ui });
    },
    columns: ['Bioma', 'Região', 'Território', 'Nome', 'Código']
  },
  'bioma_ucs_estadual': {
    name: 'Bioma_UC_Estadual',
    image: (function () {
      var rFC = ee.FeatureCollection('users/geomapeamentoipam/AUXILIAR/regioes_biomas_col2');
      var piFC = ee.FeatureCollection('projects/mapbiomas-workspace/AUXILIAR/ESTATISTICAS/COLECAO7/VERSAO-2/state_conservation_units_integral_protection').map(function (f) { return f.set('id', ee.Number.parse(f.get('feature_id'))); });
      var usFC = ee.FeatureCollection('projects/mapbiomas-workspace/AUXILIAR/ESTATISTICAS/COLECAO7/VERSAO-2/state_conservation_units_sustainable_use').map(function (f) { return f.set('id', ee.Number.parse(f.get('feature_id'))); });
      var ucs_img = ee.Image().paint(piFC, 'id').unmask(ee.Image().paint(usFC, 'id'));
      return ee.Image().paint(rFC, 'region').multiply(10000).add(ucs_img);
    })(),
    decode: function (val) {
      val = ee.Number(val); var ri = val.divide(10000).int(); var ui = val.mod(10000).int();
      var name = LEGENDS.ucs_estadual_pi.get(ui, LEGENDS.ucs_estadual_us.get(ui, 'Não identificado'));
      return ee.Dictionary({ 'Bioma': LEGENDS.biomes.get(ri.divide(10).int()), 'Região': LEGENDS.regions.get(ri), 'Território': 'UC Estadual', 'Nome': name, 'Código': ui });
    },
    columns: ['Bioma', 'Região', 'Território', 'Nome', 'Código']
  },
  'bioma_puro': {
    ...
    image: (function () {
      var rFC = ee.FeatureCollection('users/geomapeamentoipam/AUXILIAR/regioes_biomas_col2');
      return ee.Image().paint(rFC, 'region');
    })(),
    decode: function (val) {
      val = ee.Number(val);
      return ee.Dictionary({ 'Bioma': LEGENDS.biomes.get(val.divide(10).int()), 'Região': LEGENDS.regions.get(val) });
    },
    columns: ['Bioma', 'Região']
  },
  'municipio_puro': {
    name: 'Municipio',
    image: (function () {
  ...
  'uc_estadual_puro': {
    ...
    decode: function (val) {
      var name = LEGENDS.ucs_estadual_pi.get(val, LEGENDS.ucs_estadual_us.get(val, 'Não identificado'));
      return ee.Dictionary({ 'Território': 'UC Estadual', 'Nome': name, 'Código': val });
    },
    columns: ['Território', 'Nome', 'Código']
  }
};
...
var calculateArea = function (image, territory, config, spatial_config, year) {
  ...
  var areas = ee.List(data.get('groups')).map(function (terrObj) {
    ...
    var terrId = terrObj.getNumber('territory');
    var spatialProps = spatial_config.decode(terrId);

    return ee.List(terrObj.get('groups')).map(function (classObj) {
      classObj = ee.Dictionary(classObj);
      var classId = classObj.getNumber('class');
      var area_ha = ee.Number(classObj.get('sum')).multiply(100);

      var pixelProps = config.decode(classId);

      return ee.Feature(null, spatialProps.combine(pixelProps))
        .set({ 'Área ha': area_ha, 'Ano': year, 'Coleção': collection });
    });
  });
...
// ============================================================
// 8. EXECUÇÃO (Evaluate para descoberta de bandas)
// ============================================================
SUBPRODUTOS_ATIVOS.forEach(function (pid) {
  var config = DATASETS[pid];
  if (!config) return;

  // Um único evaluate por dataset ativo
  config.image.bandNames().evaluate(function (bandnames) {
    ...
    RECORTES_ATIVOS.forEach(function (sid) {
      ...
      // Filtra as bandas baseado na lista global (opcional)
      var bandsToProcess = bandnames.filter(function (b) {
        if (ANOS_FILTRO.length === 0) return true;
        var y = parseInt(b.slice(-4));
        return ANOS_FILTRO.indexOf(y) !== -1;
      });

      var all_data = bandsToProcess
        // .slice(-1)
      ...
      var final_fc = ee.FeatureCollection(all_data).flatten();
      var fileName = 'export_' + config.id + '_' + spatial_config.name;

      print('Disparando:', fileName
        // ,final_fc.limit(2)
      );
