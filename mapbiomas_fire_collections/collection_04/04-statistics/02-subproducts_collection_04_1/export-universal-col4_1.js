/** @description  - EXPORTADOR UNIVERSAL - ESTIMATIVAS COLEÇÃO 4.1 (Série Temporal)
 *
 *  Este script centraliza o cálculo de áreas queimadas para diversos subprodutos
 *  e recortes territoriais da Coleção 4.1.
 *
...
var SUBPRODUTOS_ATIVOS = [
  ...
  4,  // Frequência (Apenas Fogo - Col 4.1 não tem freq_coverage pública ainda?)
...
// 2. PARÂMETROS GERAIS
// ============================================================
var driverFolder = 'MapBiomas_Fogo_Col4_1_Estatisticas_Universal';
...
// ============================================================
// 3. CARREGAMENTO DE DADOS (Assets Públicos 4.1)
// ============================================================
var lulc = ee.Image('projects/mapbiomas-public/assets/brazil/lulc/collection9/mapbiomas_collection90_integration_v1');

var fire_monthly    = ee.Image('projects/mapbiomas-public/assets/brazil/fire/collection4_1/mapbiomas_fire_collection41_monthly_burned_v1');
var fire_annual     = ee.Image('projects/mapbiomas-public/assets/brazil/fire/collection4_1/mapbiomas_fire_collection41_annual_burned_v1');
var fire_acc        = ee.Image('projects/mapbiomas-public/assets/brazil/fire/collection4_1/mapbiomas_fire_collection41_accumulated_burned_v1');
var fire_freq       = ee.Image('projects/mapbiomas-public/assets/brazil/fire/collection4_1/mapbiomas_fire_collection41_fire_frequency_v1');
var fire_scar_size  = ee.Image('projects/mapbiomas-public/assets/brazil/fire/collection4_1/mapbiomas_fire_collection41_annual_burned_scar_size_range_v1');
var time_after_fire = ee.Image('projects/mapbiomas-public/assets/brazil/fire/collection4_1/mapbiomas_fire_collection41_time_after_fire_v1');
var year_last_fire  = ee.Image('projects/mapbiomas-public/assets/brazil/fire/collection4_1/mapbiomas_fire_collection41_year_last_fire_v1');

// Nota: fire_annual_coverage_v1 poderia ser usado, mas mantemos o cruzamento dinâmico para maior controle.

// ============================================================
// 4. LEGENDAS E AUXILIARES (MBC10 para Col 4.1)
// ============================================================
...
var LEGENDS = {
  ...
  scar_n1:   ee.Dictionary({ 1:'< 250 ha', 2:'< 250 ha', 3:'250 - 500 ha', 4:'500 - 10.000 ha', 5:'500 - 10.000 ha', 6:'10.000 - 100.000 ha', 7:'10.000 - 100.000 ha', 8:'>= 100.000 ha' }),
  scar_n2:   ee.Dictionary({ 1:'< 10 ha',  2:'10 - 250 ha', 3:'250 - 500 ha', 4:'500 - 5.000 ha', 5:'5.000 - 10.000 ha', 6:'10.000 - 50.000 ha', 7:'50.000 - 100.000 ha', 8:'>= 100.000 ha' })
};
...
var DATASETS = {
  ...
  4: {
    name: 'Frequência (Apenas Fogo)', id: 'fire_frequency',
    image: fire_freq,
...
// ============================================================
// 6. CATÁLOGO DE RECORTES (SPATIAL UNITS - Universal)
// ============================================================
var SPATIAL_UNITS = {
  'bioma_estado': {
    name: 'Bioma_Estado',
...
var calculateArea = function(image, territory, config, spatial_config, year) {
  ...
  var areas = ee.List(data.get('groups')).map(function(terrObj) {
    ...
    return ee.List(terrObj.get('groups')).map(function(classObj) {
      ...
      return ee.Feature(null, spatialProps.combine(pixelProps).set({ 'Área ha': area_ha, 'Ano': year, 'Coleção': collection }));
    });
  });
...
SUBPRODUTOS_ATIVOS.forEach(function(pid) {
  ...
  config.image.bandNames().evaluate(function(bandnames) {
    ...
    RECORTES_ATIVOS.forEach(function(sid) {
      ...
      var all_data = bandsToProcess.map(function(bandName) {
      ...
      var fileName = 'export_' + config.id + '_' + spatial_config.name + '_col41';
