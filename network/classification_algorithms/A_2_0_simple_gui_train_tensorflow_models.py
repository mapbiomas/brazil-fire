...
class TrainingInterface:
    ...
    def train_models_click(self, b):
        """
        Button callback to run the training routine.
        """
        ...
        # Extract the files that match the selection
        check_parts = active_description.split('_')
        if '✅' in check_parts[-1]:
            check_parts[-1] = check_parts[-1].replace('✅', '').strip()

        pattern = re.compile(rf"_{check_parts[3]}_.*_{check_parts[4]}\.tif")
        ...
        if selected_files:
            ...
            self.log("[INFO] No matching training files found.")
