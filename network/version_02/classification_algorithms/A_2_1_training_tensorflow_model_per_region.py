...
folder = f'/content/mapbiomas-fire/sudamerica/{country}/{collection}'  # Diretório principal da coleção

folder_samples = f'{folder}/training_samples'  # Diretório para armazenamento de dados de amostra
folder_model = f'{folder}/models'  # Diretório para armazenamento da saída dos modelos
...
class ModelTrainer:
    def __init__(self, bucket_name, country, collection, folder_model, get_active_checkbox_func):
    ...
    def train_model(self, training_data, validation_data, bi, li, data_mean, data_std, training_size):
        ...
        model_path = f'{self.folder_model}/{self.collection}_{self.country}_{version}_{region}_rnn_lstm_ckpt'
        ...
        with tf.Session(graph=graph, config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            bucket_model_path = f'gs://{self.bucket_name}/sudamerica/{self.country}/{self.collection}/models/'
            try:
...
class FileManager:
    ...
    def download_image(self, image):
        ...
        download_command = f'gsutil -m cp gs://{self.bucket_name}/sudamerica/{self.country}/{collection}/training_samples/{image} {self.folder_samples}/'
...
def sample_download_and_preparation(images_train_test):
    ...
    trainer = ModelTrainer(bucket_name, country, collection, folder_model, interface.get_active_checkbox)
