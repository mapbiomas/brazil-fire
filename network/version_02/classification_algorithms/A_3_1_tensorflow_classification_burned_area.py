...
def process_single_image(dataset_classify, version, region,folder_temp):
    ...
    gcs_model_file = f'gs://{bucket_name}/sudamerica/{country}/{collection}/models/{collection}_{country}_{version}_{region}_rnn_lstm_ckpt*'
    # Local path for the model files
    model_file_local_temp = f'{folder_temp}/{collection}_{country}_{version}_{region}_rnn_lstm_ckpt'
    ...
    json_path = f'{folder_temp}/{collection}_{country}_{version}_{region}_rnn_lstm_ckpt_hyperparameters.json'
    ...
    for satellite_year in satellite_years[:1 if simulate_test else None]:  # apenas 1 satélite se teste
        ...
        with tqdm(total=len(years), desc=f'Processing years for satellite {satellite.upper()}') as pbar_years:
            for year in years:
                ...
                gcs_filename = f'gs://{bucket_name}/sudamerica/{country}/{collection}/result_classified/{image_name}.tif'

                local_cog_path = f'{folder_mosaic}/{satellite}_{country}_{region}_{year}_cog.tif'
                gcs_cog_path = f'gs://{bucket_name}/sudamerica/{country}/{collection}/mosaics_cog/{satellite}_{country}_{region}_{year}_cog.tif'
                ...
                with tqdm(total=len(grids_to_process), desc=f'Processing scenes for year {year}') as pbar_scenes:
                    for grid in grids_to_process:
                        ...
                        output_image_name = f'{folder_temp}/image_{collection}_{country}_{region}_{version}_{orbit}_{point}_{year}.tif'
                        geometry_scene = grid['geometry']
                        NBR_clipped = f'{folder_temp}/image_mosaic_{collection}_{country}_{region}_{version}_{orbit}_{point}_clipped_{year}.tif'
